...
#endif
{
   ...
   curr->prev = 0;
   if (streamlist != 0)
      streamlist->prev = curr;
   curr->next = streamlist;
   streamlist = curr;
   if (streamtail == 0)
      streamtail = streamlist; /* the first entry */
}
...
streaminf *sepstr_new(char *tag, enum tagtype type)
    _XFUNCPROTOEND
#else
streaminf *sepstr_new(tag, type) char *tag;
enum tagtype type;
#endif
{
   streaminf *newinf;
   ...
   if (type == TAG_OUT)
      newinf->entrytype = STREAMOUT;
   if (type == TAG_IN)
      newinf->entrytype = STREAMIN;
   if (type == TAG_INOUT)
      newinf->entrytype = STREAMINOUT;
   if (type == TAG_SOCKET)
      newinf->entrytype = STREAMSOCKOUT;
   if (type == TAG_SCR)
      newinf->entrytype = STREAMSCR;
   ...
   newinf->headerbuf = 0;
   newinf->hdrlen = 0;
   newinf->hetch_queue = 0;
   newinf->hqlen = 0;
   newinf->tetch_queue = 0;
   newinf->tqlen = 0;
...
void debug_tag(char *tag)
    _XFUNCPROTOEND
#else
void debug_tag(tag) char *tag;
#endif
{
   streaminf *info;
   info = tag_info(tag, TAG_INQUIRE);
   if (info == 0)
      seperr("debug_tag request on an unintialized tag=%s \n", tag);
