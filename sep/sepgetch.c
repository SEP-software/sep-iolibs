...
_XFUNCPROTOBEGIN
int getch(const char *tag, const char *type, void *ptr) _XFUNCPROTOEND
#else
int getch(tag, type, ptr) const char *tag, *type;
char *ptr;
#endif
{
  register int iargc;
...
#if NeedFunctionPrototypes
_XFUNCPROTOBEGIN
int getch_add_string(const char *string) _XFUNCPROTOEND
#else
int getch_add_string(string) const char *string;
#endif
{
  char *copy;
  int iargc;
