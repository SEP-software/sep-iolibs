...
#if NeedFunctionPrototypes
_XFUNCPROTOBEGIN
int doc(char *name) _XFUNCPROTOEND
#else
int doc(name) char *name;
#endif
{
  FILE *s, *morein;
  int c, c1;
...
#if NeedFunctionPrototypes
_XFUNCPROTOBEGIN
char *find_doc(char *name) _XFUNCPROTOEND
#else
char *find_doc(name) char *name;
#endif
{
  ...
  if (name[0] == '/') {
    ...
    if (myl == 1)
      return name; /*not the path listed in DEFAULT_DOC_PATH*/
...
#if NeedFunctionPrototypes
_XFUNCPROTOBEGIN
char *get_doc_path(int index) _XFUNCPROTOEND
#else
char *get_doc_path(index) int index;
#endif
{
