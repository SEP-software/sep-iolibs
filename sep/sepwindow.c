...
C<int
sreed_window(tag_history,n_dim_cube,n_cube,n_wind,f_wind,j_wind,esize,values)>
...
C<int
srite_window(tag_history,n_dim_cube,n_cube,n_wind,f_wind,j_wind,esize,values)>
...
AUTHOR
   Biondo Biond1 , September 1995
         ...
         Modified: Adam Halpert : Changed "Illegal window axis" calculation (Feb
10)
...
enum {
  ...
  CREATE_GRID =
      0, /*We have a regular grid so just construct the header numbers*/
...
_XFUNCPROTOBEGIN
int sep_reed_rite_window_rec(int, const char *, const int, const int, const int,
...
#if NeedFunctionPrototypes
_XFUNCPROTOBEGIN
int sreed_window(const char *tag_history, const int *n_dim_cube,
                 const int *n_cube, const int *n_wind, const int *f_wind,
                 const int *j_wind, const int esize,
                 void *values) _XFUNCPROTOEND
#else
int sreed_window(tag_history, n_dim_cube, n_cube, n_wind, f_wind, j_wind, esize,
                 values) char *tag_history;
int *n_dim_cube;
int *n_cube;
...

{
  ...
  return (sep_window(READ, tag_history, n_dim_cube, n_cube, n_wind, f_wind,
                     j_wind, esize, values));
}

#if NeedFunctionPrototypes
_XFUNCPROTOBEGIN
int srite_window(const char *tag_history, const int *n_dim_cube,
                 const int *n_cube, const int *n_wind, const int *f_wind,
                 const int *j_wind, const int esize,
                 const void *values) _XFUNCPROTOEND
#else
int srite_window(tag_history, n_dim_cube, n_cube, n_wind, f_wind, j_wind, esize,
                 values) char *tag_history;
int *n_dim_cube;
int *n_cube;
int *n_wind, *f_wind, *j_wind;
...

{
  ...
  ierr = sep_window(WRITE, tag_history, n_dim_cube, n_cube, n_wind, f_wind,
                    j_wind, esize, buf2);
  free(buf2);
  return ierr;
}
...
#if NeedFunctionPrototypes
_XFUNCPROTOBEGIN
int sep_window(int mode, const char *tag_history, const int *n_dim_cube,
               const int *n_cube, const int *n_wind, const int *f_wind,
               const int *j_wind, const int esize, void *values) _XFUNCPROTOEND
#else
int sep_window(mode, tag_history, n_dim_cube, n_cube, n_wind, f_wind, j_wind,
               esize, values) int mode;
char *tag_history;
int *n_dim_cube;
int *n_cube;
int *n_wind, *f_wind, *j_wind;
...
#endif
{
  ...
  sep_off_t seek_to_off_t, current_pos, big_block, seek_distance,
      block_distance;
  int blocks, remainder;
  int beg_seek;        /*the ammount we need to move forward on the biggest cont
                          axies*/
  ...
  beg_seek = 0;
  for (i = 0; i < ndim; i++) {
    if (f_wind[i] < 0 || j_wind[i] <= 0 ||
        f_wind[i] + j_wind[i] * (n_wind[i] - 1) >= n_cube[i]) {
      free(ngrid);
      free(nwind);
      free(fwind);
      free(jwind);
      ...
      if (max_dim_found == NO) {
        if (block * (long long)n_wind[i] > MAX_INT_SIZE || i > last_rel) {
          ...
          last_buf_ax = i;
                   ...
                   n_cube[i]) { /*the windows spans this dimension */
          if (mode != BUFFER_READ) {
            block = (long long)block * (long long)n_wind[i];
          } else if (block * n_wind[i] <=
                     BUF_SIZE) { /*we are buffer ing and it fits*/
            ...
            last_buf_ax = i;
                   ...
                   1) { /*we have a consecutive chunk of this window*/
          if (mode != BUFFER_READ) {
            block = (sep_file_size_t)block * (sep_file_size_t)n_wind[i];
            ...
            last_buf_ax = i;
                   ...
                   (long long)BUF_SIZE >=
                       (long long)n_cube[i] * (long long)block) {
          if (mode == READ) {
            mode = BUFFER_READ;
            first_buf_ax = i;
          }
  ...
  if (mode == BUFFER_READ) {
    ...
    for (i = first_buf_ax; i < last_buf_ax; i++) {
      fbuffer[n_buf_axis] = f_wind[i];
      jbuffer[n_buf_axis] = j_wind[i];
      narray[n_buf_axis] = n_wind[i];
      nbuffer[n_buf_axis] = n_cube[i];
  ...
  for (i = 0; i < nloop; i++) {
    h2c(i, nwind, n_axis,
        ipos1); /*calculate our current position in the window*/
    ...
    if (mode == CREATE_GRID) {
      ...
      *int_pt = pos + 1;
      ...
      seek_to_off_t =
          (sep_off_t)pos * (sep_off_t)seek_block_elem + (sep_off_t)beg_seek;
...
than MAX_INT_SIZE we will do it in 2+ steps */
        ...
        file_position(tag_history, seek_block_elem * seek_block_size, &blocks,
                      &remainder);
        current_pos = big_block * (sep_off_t)blocks + (sep_off_t)remainder;
        seek_distance = seek_to_off_t * seek_block_size - current_pos;
        block_distance = seek_distance / (sep_off_t)seek_block_size; /*units to
relative seek in seek_block_size quantities*/
        /*      fprintf(stderr,"now time to check %g %g %g \n",(long
         * long)current_pos,(long long)seek_distance,block_distance);*/
        if ((size_t)block_distance > MAX_INT_SIZE) { /*if larger than max int */
          ...
          if (derr < 0)
            return (sepwarn(-1,
                            "trouble seeking %d blocks of size %d in tag %d\n",
      ...
      if (derr < 0) {
        ...
        fprintf(stderr,
                "sepwindow, SEEK ERROR: big_block=%d cur=%17.12g \n "
                "seek_to_off=%17.12g  block_distance=%17.12g \n",
                (int)big_block, (double)current_pos, (double)seek_to_off_t,
                (double)block_distance);
        return (sepwarn(-1,
                        "trouble seeking(2) %d blocks of size %d in tag %s\n",
      ...
      if (mode == WRITE) {
        ...
        if (block != ierr) {
          ...
          free(ipos1);
          return (
              sepwarn(-1, "sreed  buffer error for tag %s \n", tag_history));
        }
        for (ia = 0; ia < nloop_buf; ia++) {
          ...
          c2h(&pos2, nbuffer, n_buf_axis,
              ipos2); /* convert to grid parameters */
                      /*				fprintf(stderr,"buff looop from=%d to=%d
                       * \n",ia,pos2);*/
          memcpy((void *)pval, (const void *)(buffer + copy_size * pos2),
                 copy_size);
...
void compress_it(const int ndim, const int *ncube, const int *nwind,
                 const int *fwind, const int *jwind, const int esize,
                 wind *comp, int type);
...
int sreed_window_new(const char *tag_history, const int ndim, const int *n_cube,
                     const int *n_wind, const int *f_wind, const int *j_wind,
                     const int esize, void *values) {
  wind compress;
...
int srite_window_new(const char *tag_history, const int ndim, const int *n_cube,
                     const int *n_wind, const int *f_wind, const int *j_wind,
                     const int esize, const void *values) {
  size_t n;
  char *buf2;
  int i, ierr;
  n = esize;
              ...
              for (i2 = 0; i2 < pars->nw[2]; i2++) {
                ...
                for (i1 = 0; i1 < pars->nw[1]; i1++) {
                  ...
                  for (i0 = 0; i0 < pars->nw[0]; i0++) {
                    seek_it = seek_to[1] +
                              (pars->fw[0] + pars->jw[0] * i0) * pars->bs[0];
                    ...
                    if (write == 1) {
                      if ((int)pars->block !=
                          srite(tag, &buffer[pos], (int)pars->block)) {
                        fprintf(
                            stderr,
                            "Trouble writing tag=%s file_pos=%g buffer=%g\n",
                      ...
                      if (pars->nit == -1) {
                        if ((int)pars->block !=
                            sreed(tag, &buffer[pos], (int)pars->block)) {
                          fprintf(
                              stderr,
                              "Trouble reading tag=%s file_pos=%g buffer=%g\n",
                              tag, (double)seek_it, (double)pos);
                          return 1;
                        }
                        pos += pars->block;
                        ...
                        if ((int)pars->block !=
                            sreed(tag, wind_buf, (int)pars->block)) {
                          fprintf(
                              stderr,
                              "Trouble reading tag=%s file_pos=%g buffer=%g\n",
                              tag, (double)seek_it, (double)pos);
...
void compress_it(const int ndim, const int *ncube, const int *nwind,
                 const int *fwind, const int *jwind, const int esize,
                 wind *comp, const int type) {
  wind big;
  long long big_int =
      (long long)1024 * (long long)1024 * (long long)1024 * 2 - 1;
  int idim = 0;
  int found = 0;
  int first = 0;
  ...
  for (i = 0; i < ndim; i++) {
    ...
    if (fwind[i] < 0 || jwind[i] < 1 ||
        (fwind[i] + 1) + jwind[i] * (nwind[i] - 1) > ncube[i])
      seperr("Illegal window axis=%d ncube=%d nw=%d fw=%d jw=%d \n", i,
             ncube[i], nwind[i], fwind[i], jwind[i]);
    big.nc[idim] = ncube[i];
    big.nw[idim] = nwind[i];
  ...
  while (found == 0 && i < nd) {
    if (big.nc[i] == big.nw[i]) {
      ...
      if (block > big_int) {
        found = 1;
        comp->block = block / ((long long)big.nw[i]);
      }
    } else {
      found = 1;
      comp->block = block;
  ...
  if (type == 0 && block < 100 &&
      (long long)block * (long long)big.nc[0] < myalloc && big.nw[0] > 5) {
    comp->jit = big.jw[0];
    comp->nit = big.nw[0];
    comp->fit = big.fw[0];
    comp->bit = block;
    block = block * big.nc[0];
    nd--;
