...
#if NeedFunctionPrototypes
_XFUNCPROTOBEGIN
int srite(const char *tag, void *buf, int nbytes) _XFUNCPROTOEND
#else
int srite(tag, buf, nbytes) char *tag;
char *buf;
int nbytes;
#endif
{
  ...
  if (info->ioinf == 0) {
    ...
    if (!info->valid) {
      seperr("srite(): invalid output tag %s\n", tag);
    }
  }
...
#if NeedFunctionPrototypes
_XFUNCPROTOBEGIN
int srite_raw(char *tag, void *buf, int nbytes) _XFUNCPROTOEND
#else
int srite_raw(tag, buf, nbytes) char *tag;
void *buf;
int nbytes;
#endif
{
...
#if NeedFunctionPrototypes
_XFUNCPROTOBEGIN
int srite2(char *tag, void *buf, int nbytes, char *format) _XFUNCPROTOEND
#else
int srite2(tag, buf, nbytes, format) char *tag;
char *buf;
int nbytes;
char *format;
#endif
...
#if NeedFunctionPrototypes
_XFUNCPROTOBEGIN
static int srite_xdr(streaminf *info, void *buf, int nbytes,
                     int format_num) _XFUNCPROTOEND
#else
static int srite_xdr(info, buf, nbytes, format_num) streaminf *info;
void *buf;
int nbytes;
int format_num;
#endif
{
  ...
  do {
    size = MIN(nwrite - total,
               XDRSIZE - 4); /*REMOVE THE -4 WHEN LINUX XDR_BYTES WORKS*/
    xdr_setpos(outxdr, 0);
    ...
    switch (format_num) {
...
#if defined(CRAY)
      {
        ...
        type = 2;
        ierr =
            CRAY2IEG(&type, &numel, (float *)outxdrbuf, &bitoff, (float *)cpos);
        if (ierr != 0) {
...
#else
        if (xdr_vector(outxdr, cpos, size / 4, sizeof(float),
                       (xdrproc_t)xdr_float) == FALSE) {
        ...
        break;
      case (FMT_XDR_INT):
        if (xdr_vector(outxdr, cpos, size / 4, sizeof(int),
                       (xdrproc_t)xdr_int) == FALSE) {
          seperr("xdr_rite(): xdr error, tag \"%s\" \n", info->tagname);
        }
...
_XFUNCPROTOBEGIN
size_t srite_big(const char *tag, void *buf, size_t nbytes) _XFUNCPROTOEND
