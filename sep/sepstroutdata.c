...
#if NeedFunctionPrototypes
_XFUNCPROTOBEGIN
void open_outstream(streaminf *info) _XFUNCPROTOEND
#else
void open_outstream(info) streaminf *info;
#endif
{
...
#if NeedFunctionPrototypes
_XFUNCPROTOBEGIN
static char *expand_name(char *file) _XFUNCPROTOEND
#else
static char *expand_name(file) char *file;
#endif
{
...
#if NeedFunctionPrototypes
_XFUNCPROTOBEGIN
char *sep_tail(char *name) _XFUNCPROTOEND
#else
char *sep_tail(name) char *name;
#endif
{
  char *result;
...
#if NeedFunctionPrototypes
_XFUNCPROTOBEGIN
static char *makenames(char *path, char *tail, int randit) _XFUNCPROTOEND
#else
static char *makenames(path, tail, randit) char *path;
char *tail;
int randit
#endif
{
  ...
  if (ierr == 1) {
    ...
    strcpy(path_use, path);
  all_paths = strcpy((char *)malloc((int)strlen(path_use) + 1), path_use);
  all_names = (char *)malloc(((int)strlen(all_paths) + (int)strlen(tail) + 4) *
                                 MAX_SEP_FILE +
                             1); /* the +4 is file number + ; + @ */

  /*
          The general idea for multi-file writes is to grab the list of
directories to write to and the size of files to write. We default to
SEP_FILE_SIZE per drive and file size of SEP_FILE_SIZE (which can be overwritten
by "dirsize" and "filesize".  We  then create a new datapath containing an
expanded list of names. The multi-file option can be initialized by either a
';' in the datapath list or by a filesize < dirsize
*/

  if (0 == getch("nfiles", "d", &nfiles))
    nfiles = 1;
  if (nfiles > MAX_SEP_FILE)
    seperr("nfiles to large, max %d \n", MAX_SEP_FILE);
  dirsize[0] = SEP_DIR_SIZE;
  filesize[0] = SEP_FILE_SIZE;
  ncount = getch("dirsize", "d", &dirsize);
  for (i = ncount; i < nfiles; i++)
    dirsize[i] = dirsize[MAX(ncount - 1, 0)];
  ncount = getch("filesize", "d", &filesize);
  for (i = ncount; i < nfiles; i++)
    filesize[i] = filesize[MAX(ncount - 1, 0)];
...
#if NeedFunctionPrototypes
_XFUNCPROTOBEGIN
static void stdoutname(streaminf *info) _XFUNCPROTOEND
#else
static void stdoutname(info) streaminf *info;
#endif
{
    ...
    if (info->isapipe || isapipe(fileno(info->headfile))) {
      ...
      if (!strcmp(datapath(datapth), "stdout")) {
        ...
        if ((getch("head", "s", parambuf4) && isordinary(parambuf4)) ||
            (0 <
             findnm(fileno(info->headfile), parambuf4, sizeof(parambuf4)))) {
          /* modify slightly */
          ...
          i = 0;
          j = 0;
          while (getSepArgV()[0][i] != '\0') {
            if (getSepArgV()[0][i] == '/')
              j = 0;
            else {
              prog_name[j] = getSepArgV()[0][i];
              j++;
            }
            i++;
          }
...
#if NeedFunctionPrototypes
_XFUNCPROTOBEGIN
int get_data_name(char *tag, char *string_out) _XFUNCPROTOEND
#endif
{
  streaminf *info;
  char *name;
  ...
  if (strchr(info->headername, '|') != 0)
    return (1);
  else if (strchr(info->headername, ':') != 0)
    return (1);
    ...
    strncpy(junk, sep_tail(name), sizeof(junk) - 1);
    junk[PATH_MAX - 1] = '\0';
    strcpy(string_out, junk);
    i = 0;
    free(name);
    return (0);
  }
...
#if NeedFunctionPrototypes
_XFUNCPROTOBEGIN
void mkrandom_string(char *string_in, char *string_out) _XFUNCPROTOEND
#endif
{
  time_t tempt;
  int put_end, i, j;
...
#if NeedFunctionPrototypes
_XFUNCPROTOBEGIN
void outname(streaminf *info) _XFUNCPROTOEND
#else
void outname(info) streaminf *info;
#endif
{
