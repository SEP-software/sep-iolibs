...
#if NeedFunctionPrototypes
_XFUNCPROTOBEGIN
static void make_queues(streaminf *info) _XFUNCPROTOEND
#else
static void make_queues(info) streaminf *info;
#endif
{
   ...
   */
  if (getSepArgC() == 0)
    seperr("Must call initpar() at the start of your program\n");

  /*Commented for now because reg_ex slowness on LINUX and tetch not used */
  /* headx=tetch_chop(*,info->headerbuf); */
  ...
  if (*headx) {
    ...
    if (info->headfile)
      if (isatty(fileno(info->headfile)))
        return;
...
#if NeedFunctionPrototypes
_XFUNCPROTOBEGIN
int sepstrpar(streaminf *info, char *tag, char *type, void *val) _XFUNCPROTOEND
#else
int sepstrpar(info, tag, type, val) streaminf *info;
char *tag, *type;
void *val;
#endif
{
  ...
  assert(info->entrytype == STREAMIN || info->entrytype == STREAMINOUT ||
         info->entrytype == STREAMSCR);

  if (info->headerbuf == 0 && info->entrytype == STREAMIN) {
    seperr("Attempt to get parameter %s from tag %s which is not a valid "
           "description file \n",
           tag, info->tagname);
  }
...
#if NeedFunctionPrototypes
_XFUNCPROTOBEGIN
static char *tetch_chop(char *command, char *history) _XFUNCPROTOEND
#else
static char *tetch_chop(command, history) char *command, *history;
#endif
{
  char expression[128];
  char newcommand[128];
...
#define SPECIAL "\\[.^*$"
  ...
  while (*ptr) { /* then command name */
    if (((char *)NULL) != strrchr(SPECIAL, *ptr))
      *(eptr++) = '\\';
    *(eptr++) = *(ptr++);
  }
