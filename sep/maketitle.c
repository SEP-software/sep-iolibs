...
_XFUNCPROTOBEGIN
char *maketitle(char *string) _XFUNCPROTOEND
#else
char *maketitle(string) char *string;
#endif
{
  ...
  pwnam[0] = '\0';
  statstr = getlogin();
  if (statstr != (char *)NULL)
    strcpy(pwnam, statstr);
  if (pwnam[0] == '\0') {
    uident = getpwuid(geteuid());
    if (uident != (struct passwd *)NULL)
      strcpy(pwnam, uident->pw_name);
  }
  ...
  sprintf(string, "\n\n%s:   %s@%s   %s", getSepArgV()[0], pwnam, hostnam,
          ctime(&tloc));
  return (string);
}
