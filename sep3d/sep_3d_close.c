...
_XFUNCPROTOBEGIN
void sep_3d_close(void) _XFUNCPROTOEND
#else
void sep_3d_close()
#endif
{
...
AN ERROR */
        seperr("Can not call auxclose on the headers (most likely by writing "
               "out the headers) before sep_3d_close if you wish to pipe\n");
      }
...
HAS ALREADY BEEN WRITTEN OUT THE SOCKET.  FOR NOW JUST GIVE
AN ERROR */
        seperr("Can not call auxclose on the grid (most likely by writing out "
               "the headers) before sep_3d_close if you wish to pipe\n");
      }
      /*NOW WE NEED TO DO SOME MAGIC, FIRST CREATE*/
      auxsockout("gff_out");
  ...
  timeout = 5;
  getch("timeout", "i", &timeout);

  if (hff == 1) {
    if ((info_hff_out->sockfd = socklisten(info_hff_out->sockfd, timeout)) ==
        -1)
...
HFF FILE */
    seekpos = (off_t)0;
    whence = 0;
    if (0 != fseek(info_hff_in->headfile, (long)seekpos, whence))
      perror("file_seek");
  ...
  if (gff == 1) {
    if ((info_gff_out->sockfd = socklisten(info_gff_out->sockfd, timeout)) ==
        -1)
...
HFF FILE */
    seekpos = (off_t)0;
    whence = 0;
    if (0 != fseek(info_gff_in->headfile, (long)seekpos, whence))
      perror("file_seek");
